...
> AI Agent 桌面端的皮肤、主题与换肤工具精选列表 — ChatGPT/Codex、Claude、Cursor、Antigravity 等。

[English](README.md) | 简体中文

你的 AI Agent 是你每天盯几个小时的应用。Agent 皮肤让你给它的界面换脸——配色、壁纸、角色、完整视觉身份——不碰官方安装包，不动你的 API 配置。

范围：本列表只收 **AI Agent**——编程类 agent（Codex、Claude、Cursor、Antigravity、Windsurf）优先，再到更广的 AI 助手桌面端。这不是通用编辑器/终端美化列表；每个条目都服务于"给 AI agent 换肤"。
...
<sup>运行时换肤后的 Codex 桌面端。截图来自 Codex-Dream-Skin 项目（软件为 MIT 协议；图中视觉素材的权利需另行核验）。</sup>
...
- [跨宿主与元项目](#跨宿主与元项目)
- [宠物、头像与陪伴体](#宠物头像与陪伴体)
- [Web Agent（Userstyles）](#web-agentuserstyles)
- [注入引擎与框架](#注入引擎与框架)
- [画廊与市场](#画廊与市场)
- [讨论](#讨论)
- [词义辨析](#词义辨析)
...
| 宿主                 | 内核                     | CDP 注入           | 原生主题               |
| -------------------- | ------------------------ | ------------------ | ---------------------- |
| ChatGPT/Codex 桌面端 | Electron（已合并为一个） | ✅（震中）         | ✅（`codex-theme-v1`） |
| Claude Desktop       | Electron                 | 受限（见章节说明） | ❌                     |
...
- [Finderchangchang/codex-autoskin](https://github.com/Finderchangchang/codex-autoskin) - 实验性：`theme.json` + 素材 manifest + `THEME-SPEC.md`，一张图生成主题，CDP 注入可还原。非常新——验证了"生成皮肤包"这个方向。
...
Electron 应用，注入技术上可行——但这块几乎无人开发。注意：近期 Claude Desktop 构建似乎限制了 CDP 依赖的远程调试参数，Codex 式 CDP 换肤可能需要 Electron fuse/ASAR 级别的手段（研究性质，不是普通安装）。Claude Desktop 本身只有 Light/System/Dark 和聊天字体选项。（Claude _Code_ 终端应用有完整的官方 JSON 主题——那是另一个表面。）
...
跨平台的 Claude Desktop CDP 换肤工具至今不存在——生态最大的空缺（且当前构建能否注入尚待确认）。
...
两个不同的表面：**Editor/IDE** 是 VS Code fork（Open VSX，标准配色/图标主题适用）；**Desktop agent 指挥中心**是独立 UI，没有证据表明 VS Code 主题能覆盖它。目前只有原生主题——没有 Antigravity 专属注入工具（通用 VS Code loader 适用于 Editor）。

- [nik-holo/montexia-theme](https://github.com/nik-holo/montexia-theme) - 面向 Cursor、Antigravity、Windsurf 的深色主题。
- [DavidBabel/antigravity-icons-supercharged](https://github.com/DavidBabel/antigravity-icons-supercharged) - Antigravity / VS Code 家族的 product icon 主题。
...
视觉个性化的平行分支：不换整张脸，而是给 agent 加一只动画陪伴体。ChatGPT 官方的 [Pets](https://learn.chatgpt.com/docs/pets) 功能（内置 `hatch-pet` skill：按你的描述生成宠物并保存在本地）在平台层面验证了"生成+收集"模式。

- [crafter-station/petdex](https://github.com/crafter-station/petdex) - AI agent 宠物的画廊 + CLI + 桌面浮窗（3k+ star）。`pet.json` + spritesheet 包格式，hatch/install/select/submit/doctor 全流程——现存最接近"皮肤画廊+生成器+市场"闭环的样板。

## Web Agent（Userstyles）

浏览器版的 AI 聊天 agent 可以用 userstyles（Stylus/userContent CSS）换肤。

- [catppuccin/userstyles](https://github.com/catppuccin/userstyles) - ChatGPT、Claude、Gemini、Perplexity 等的 userstyles。Web CSS 注入——与桌面原生主题是两回事。

## 注入引擎与框架

上面这些工具背后可复用的底层机制，给想自己造工具的人。CDP 方案：以绑定 loopback 的 `--remote-debugging-port` 启动 agent 的 Electron 应用，通过 Chrome DevTools Protocol 驱动——Dream-Skin 及其后代的实现机制。asar 补丁方案：通过 Electron `webContents.insertCSS()` 注入 CSS（claude-desktop-bin 采用）。VS Code 系 loader 则给 `workbench.html` 打补丁。

- [magicdawn/vsc-custom-css-cli](https://github.com/magicdawn/vsc-custom-css-cli) - VS Code 自定义 CSS loader 的 CLI 版，可用于给 Cursor/Windsurf/Antigravity 换肤。
- [Codex++ 解析文章](https://we0.ai/articles/codex-plus-plus-tutorial-codex-desktop-enhancement) - 用同一套 CDP 机制向 Codex 注入行为级增强的第三方启动器（Tauri/React + 用户脚本）。
...
- [cnyzgkc 的 X 帖](https://x.com/cnyzgkc/status/2077642188599759193) - 社区 meme 天花板：有人把 OpenAI 的 Codex 换成了 Anthropic CEO 的文艺风定制皮肤。

<img src="media/dario-tribute-skin.jpg" alt="Codex 换上 Dario Amodei 文艺定制皮肤" width="600">

<sup>Agent 皮肤 meme 文化的巅峰。截图来自上方链接的帖子。</sup>
